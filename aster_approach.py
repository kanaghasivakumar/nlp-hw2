...
# Import the model architecture from the starter code
        ...
        with open(file_path, 'rt', encoding='utf-8') as f:
            for line in f:
                ...
                if task == "classification":
                    # For Task 2: Fine-tune on the correct sequence
                    correct_choice = choices[answer_key]
                    text = f"{fact} {stem} {correct_choice}"
                else:
                    # For Task 3 & 4: Fine-tune to generate the answer text
                    correct_choice = choices[answer_key]
...
class AsterPipeline:
    ...
    def __init__(self, tokenizer_dir="saved/tokenizer", model_path="saved/model_best.pt"):
        ...
        # Initialize with starter code defaults
    ...
    def fine_tune(self, train_loader, valid_loader, epochs=5, patience=2, save_name="aster_finetuned.pt"):
        """
        Executes autoregressive fine-tuning with early stopping and loss plotting.
        """
    ...
    def sequence_probability(self, text):
        """
        Calculates the log probability of a specific sequence for Task 2.
        """
        self.model.eval()
    ...
    def compute_bertscore(self, text1, text2):
        ...
        # Access the embedding layer from TransformerGPT
        embs1 = self.model.wte(ids1).squeeze(0) 
        embs2 = self.model.wte(ids2).squeeze(0)
        
        # Mean pooling for simplicity, can be updated to exact BERTScore alignment
    ...
    def beam_search(self, prompt, beam_width=3, max_len=15, guided=False):
        ...
        for _ in range(max_len):
            ...
            for seq, score in beams:
                ...
                for i in range(beam_width):
                    ...
                    if guided:
                        ...
                        # Composite score: hyperparameter weights can be adjusted
            ...
            # Stop if the top beam generates an EOS token
...
def execute_evaluation(pipeline, dataset, output_log="generation_logs.txt"):
    ...
    for i in tqdm(range(total), desc="Evaluating"):
        ...
        # Collect logs for the report requirement
...
def main():
    # Initialize pipeline with pre-trained weights
    pipeline = AsterPipeline()
    
    # 1. Evaluate Zero-Shot
